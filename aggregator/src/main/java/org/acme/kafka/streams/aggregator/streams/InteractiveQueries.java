...
public class InteractiveQueries {
    ...
    public QueryResult getPlayerMatchesStore(String id) {
        ...
        if (metadata == null || metadata == KeyQueryMetadata.NOT_AVAILABLE) {
            ...
            LOG.infov("Found data for key {0} on remote host {1}:{2}", id, metadata.getActiveHost().host(), metadata.getActiveHost().port());
            return QueryResult.foundRemotely(metadata.getActiveHost().host(), metadata.getActiveHost().port());
        }
    }
    ...
    private ReadOnlyKeyValueStore<String, String> getPlayerMatchesStore () {
        while (true) {
            try {
                ...
                return streams.store(StoreQueryParameters.fromNameAndType(TopologyProducer.MATCHES_STORE, QueryableStoreTypes.keyValueStore()));
